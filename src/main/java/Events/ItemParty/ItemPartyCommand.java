...
public class ItemPartyCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (subCommand) {
            ...
            case "add":
                if (args.length < 2) {
                    sender.sendMessage(ChatColor.RED + "Uso: /itemparty add <jugador>");
                    return true;
                }
                itemPartyHandler.gestionarParticipantes(sender, "add", args[1]);
                break;
            case "remove":
                if (args.length < 2) {
                    sender.sendMessage(ChatColor.RED + "Uso: /itemparty remove <jugador>");
                    return true;
                }
                itemPartyHandler.gestionarParticipantes(sender, "remove", args[1]);
                break;
            case "castigo":
    ...
    private void sendUsage(CommandSender sender) {
        sender.sendMessage(ChatColor.RED + "Uso: /itemparty <start|end|reset|reload|add|remove|castigo>");
    }

    @Override
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (args.length == 1) {
            List<String> options = Arrays.asList("start", "end", "reset", "reload", "add", "remove", "castigo");
