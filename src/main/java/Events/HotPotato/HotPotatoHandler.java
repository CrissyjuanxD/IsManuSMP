...
public class HotPotatoHandler implements Listener {
    ...
    private BukkitTask taskDetectorPoderes;
    private BukkitTask taskReduccionBordeContinuo;

    // --- NUEVO: BORDE VIRTUAL ---
    private WorldBorder borderFalso;
    ...
    // --- NUEVAS CONFIGS ---
    private boolean bordeActivado = true;
    private int maxRondasConfig = 10;
    ...
    public void crearYcargarConfig() {
        ...
        config.options().header(
                "==========================================================\n" +
                        ...
                        "borde_activado: Habilita el WorldBorder falso visual y de daño.\n" +
                        "max_rondas: Límite de rondas para el modo 'max'.\n" +
                        "modo_rondas:\n" +
                        "  'rapida' -> Escala la cantidad de bombas en +1 por ronda.\n" +
                        "  'lenta'  -> Sube, se mantiene, sube, se mantiene.\n" +
                        "  'max'    -> Basa el ritmo en el parámetro 'max_rondas'.\n" +
                        "=========================================================="
        );
        ...
        if (!config.contains("tiempo_por_ronda_segundos")) {
            ...
            config.set("borde_activado", true);
            config.set("max_rondas", 10);

            config.set("zona.minX", 20913);
            config.set("zona.maxX", 21088);
            config.set("zona.minY", 72);
            config.set("zona.maxY", 116);
        ...
        bordeActivado = config.getBoolean("borde_activado", true);
        maxRondasConfig = Math.max(2, config.getInt("max_rondas", 10)); // Minimo 2 rondas

        arenaMinX = config.getInt("zona.minX", 20913);
        arenaMaxX = config.getInt("zona.maxX", 21088);
        arenaMinY = config.getInt("zona.minY", 72);
        arenaMaxY = config.getInt("zona.maxY", 116);
        ...
        double espY = config.getDouble("espectadores.y", 101.0);
    ...
    private void inicializarTeamHotPotato() {
        ...
        if (t == null) t = mainBoard.registerNewTeam(Handlers.Teams.TeamType.HOTPOTATO.getId());
    ...
    private void aplicarTeamHotPotato(Player p) {
        ...
        originalTeams.put(p.getName(), oldTeam != null ? oldTeam.getName() : null);
    ...
    public void iniciarEvento() {
        ...
        if (segundosRestantesTimer > 0) tiempoText += " y " + segundosRestantesTimer + " segundos";
    ...
    private void teletransportarJugadores() {
        ...
        for (String nombre : participantes) {
            ...
            if (p != null) {
                if (eventInventoryManager != null) eventInventoryManager.saveAndClearInventory(p);
    ...
    public void iniciarBatalla() {
        ...
        // --- CREAR EL BORDE FALSO SI ESTÁ ACTIVADO ---
        if (bordeActivado) {
            borderFalso = Bukkit.createWorldBorder();
            borderFalso.setCenter((arenaMinX + arenaMaxX) / 2.0, (arenaMinZ + arenaMaxZ) / 2.0);
            borderFalso.setSize(Math.max(arenaMaxX - arenaMinX, arenaMaxZ - arenaMinZ));
            borderFalso.setDamageAmount(0); // El daño lo calculamos nosotros
            borderFalso.setDamageBuffer(0);
            borderFalso.setWarningDistance(1);
        }
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (contador > 0) {
                    ...
                    iniciarDetectorPoderes(); // OPTIMIZADO: Sacado del PlayerMoveEvent
                    iniciarRotacionPoderes(); // OPTIMIZADO: Quaternions de 1 tick a 20 ticks
    ...
    private void calcularRondas() {
        ...
        if (total <= 3) {
            ...
            if (velocidadRondas.equals("max")) {
                ...
                int rondasParaEliminar = maxRondasConfig - 1; // Dejamos la ultima ronda para los 3 finalistas

                if (cantidadEliminar <= rondasParaEliminar) {
                    for (int i = 0; i < cantidadEliminar; i++) {
                        secuenciaBombas.add(1);
                    }
                } else {
                    int basePorRonda = cantidadEliminar / rondasParaEliminar;
                    int residuo = cantidadEliminar % rondasParaEliminar;

                    for (int i = 0; i < rondasParaEliminar; i++) {
    ...
    private void siguienteRonda() {
        ...
        // --- ASIGNAR BORDE A LOS JUGADORES EN ZONA ---
        if (bordeActivado && borderFalso != null) {
            for (Player p : getJugadoresEnZona()) {
                p.setWorldBorder(borderFalso);
            }
        }
    ...
    private void procesarMuerte(Player jugador, Player atacante, String causa) {
        ...
        if (vivos.contains(eliminado)) {
            ...
            jugador.setWorldBorder(null); // Quitar el borde falso al morir
    ...
    // --- NUEVO: DETECTOR DE PODERES OPTIMIZADO (Sacado del PlayerMoveEvent) ---
    private void iniciarDetectorPoderes() {
        taskDetectorPoderes = new BukkitRunnable() {
            @Override
            public void run() {
                if (!enBatalla || !poderesActivados || rondaEnPausa) return;

                for (String pName : vivos) {
                    Player p = Bukkit.getPlayer(pName);
                    if (p == null || bombasActuales.contains(p.getName())) continue;

                    Iterator<Entity> iter = poderesEnSuelo.iterator();
                    while (iter.hasNext()) {
                        Entity ent = iter.next();
                        if (ent.isValid() && ent.getLocation().distanceSquared(p.getLocation()) < 2.5) {
        ...
        }.runTaskTimer(plugin, 0L, 5L); // Ejecuta cada 1/4 de segundo (5 ticks)
        tareasActivas.add(taskDetectorPoderes);
    }
    ...
    // EL EVENTO ONMOVE YA NO CALCULA DISTANCIAS, CPU SALVADO
    @EventHandler
    public void onMove(PlayerMoveEvent e) {
        // Se ha vaciado porque ahora 'taskDetectorPoderes' maneja esto asíncronamente/por ticks
    }
    ...
    public void onInventoryClick(InventoryClickEvent e) {
        if (enBatalla && vivos.contains(e.getWhoClicked().getName())) e.setCancelled(true);
    }

    @EventHandler
    public void onPlayerDrop(PlayerDropItemEvent e) {
        if (enBatalla && vivos.contains(e.getPlayer().getName())) e.setCancelled(true);
    }
    ...
    // --- NUEVO: ROTACION DE ITEMS CON QUATERNIONS INTERPOLADOS ---
    private void iniciarRotacionPoderes() {
        taskRotacionPoderes = new BukkitRunnable() {
            ...
            public void run() {
                ...
                angle += (float) Math.PI; // Da un giro de 180° cada vez que se llama
                if (angle > Math.PI * 2) angle -= Math.PI * 2;

                for(Entity ent : poderesEnSuelo) {
                    if(ent instanceof ItemDisplay display && ent.isValid()) {
                        // Forzar al cliente a interpolar la animación durante los siguientes 20 ticks (1 segundo)
                        display.setInterpolationDelay(0);
                        display.setInterpolationDuration(20);

                        org.bukkit.util.Transformation transform = display.getTransformation();
                        transform.getLeftRotation().rotationY(angle); // Modificamos el quaternion nativamente
                        display.setTransformation(transform);
                    }
                }
            }
        }.runTaskTimer(plugin, 0L, 20L); // Llamamos al server CADA SEGUNDO en vez de CADA TICK
    ...
    public void terminarEvento() {
        ...
        for (String pName : participantes) {
            ...
            if (p != null) {
                p.getInventory().clear();
                if (eventInventoryManager != null) eventInventoryManager.restoreInventory(p);
    ...
    private void inicializarScoreboard() {
        ...
        for (Player p : getJugadoresEnZona()) {
            ...
            for (Team mainTeam : mainBoard.getTeams()) {
                ...
                for (String entry : mainTeam.getEntries()) newTeam.addEntry(entry);
            }
    ...
    // --- NUEVO: REDUCCION INTEGRADA AL BORDE VIRTUAL ---
    private void iniciarReduccionBordeContinuo(int cX, int cZ) {
        if (bordeActivado && borderFalso != null) {
            // El API de WorldBorder reduce todo de forma suave durante X segundos (10s en tu caso).
            borderFalso.setSize(24, 10L);
        }
        ...
        if (eventoIniciado) {
            for (World world : Bukkit.getWorlds()) {
                for (Entity entity : world.getEntities()) {
                    if (entity instanceof Monster mob && isInsideArena(entity.getLocation())) mob.remove();
                }
            }
        }
    }
    ...
    private void iniciarLimpiezaDeMobs() {
        BukkitTask taskLimpieza = new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (world != null) {
                    for (Entity entity : world.getEntities()) {
                        if (entity instanceof Mob && isInsideArena(entity.getLocation())) entity.remove();
                    }
                }
            }
    ...
    private void cancelarTareasActivas() {
        for (BukkitTask tarea : tareasActivas) {
            if (tarea != null && !tarea.isCancelled()) tarea.cancel();
        }
        tareasActivas.clear();
    }
