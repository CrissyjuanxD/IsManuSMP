    ...
    public class DayOneChanges implements Listener {
        ...
        public void onCreatureSpawn(CreatureSpawnEvent event) {
            ...
            if (currentDay >= 5) {
                ...
                zombieProbability = 15;
                spiderProbability = 15;
            }
        ...
        public void onPortalEnter(PlayerPortalEvent event) {
            if (isApplied) {
                DayHandler dayHandler = ((IsManuSMP) plugin).getDayHandler();
                if (dayHandler.getCurrentDay() < 4 && event.getCause() == PlayerPortalEvent.TeleportCause.NETHER_PORTAL) {
                    event.setCancelled(true);
                    event.getPlayer().sendMessage(ChatColor.RED + "۞ El Nether está deshabilitado.");
                }
            }
        }
        ...
        public void onEndEnter(PlayerPortalEvent event) {
            if (isApplied) {
                DayHandler dayHandler = ((IsManuSMP) plugin).getDayHandler();
                if (dayHandler.getCurrentDay() < 5 && event.getCause() == PlayerPortalEvent.TeleportCause.END_PORTAL) {
                    event.setCancelled(true);
                    event.getPlayer().sendMessage(ChatColor.RED + "۞ El End está deshabilitado.");
                }
            }
        }
