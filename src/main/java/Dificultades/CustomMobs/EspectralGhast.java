...
import Dificultades.Features.EspectralMobsTP;
import org.bukkit.*;
...
public class EspectralGhast extends EspectralMobsTP implements Listener {
    private static boolean eventsRegistered = false;

    public EspectralGhast(JavaPlugin plugin) {
    ...
    private void applyEnderGhastAttributes(Ghast ghast) {
        ghast.setCustomName(ChatColor.LIGHT_PURPLE + "" + ChatColor.BOLD + "Spectral Ghast");
        ghast.setCustomNameVisible(false);
        ghast.getAttribute(Attribute.MAX_HEALTH).setBaseValue(60);
        ghast.setHealth(60);
        ghast.getAttribute(Attribute.FOLLOW_RANGE).setBaseValue(96);
        ghast.getAttribute(Attribute.SCALE).setBaseValue(0.8);
